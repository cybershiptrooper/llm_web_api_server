...
def test_generate_db_from_pdf():
    pdf_path = os.path.join(os.path.dirname(__file__), os.join(pdfs_dir, "CS_781_Project.pdf"))
    generator = ContextBasedGenerator(pdf_path)
    reponse = generator.generate_chain_response("Make an mcq quiz form with three questions in it.")
    try:
